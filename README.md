...
**Версія 1.0.18** | Внутрішній інструмент керування кінцевими точками
...
| **Увімкнення BitLocker** | Перевіряє статус диска C:. Якщо `FullyDecrypted`, вмикає BitLocker із протектором TPM-only (`XtsAes256`). Автоматично генерує та додає протектор `RecoveryPassword` і відправляє його в Infisical Cloud. |
| **Ескроу recovery-ключа** | Отримує 48-значний пароль відновлення BitLocker і надійно завантажує його до Infisical Cloud (у версії 1.0.18 надсилання ключа інтегровано безпосередньо в основний крок BitLocker). |
...
| Компонент | Версія |
|---|---|
| hard-duck.ps1 | 1.0.18 |
| Hard-Duck.exe | 1.0.18 |
